package nl.five.timestretch.compact;


public class TimeStretcherCompact {
	...
	public TimeStretcherCompact(float[] sourceBuffer, float sampleRate, float grainSizeSecs) {
		...
		for(int i = 0; i < volumeEnvelope.length; i++) {
			...
			volumeEnvelope[i] = 1.0F - (float) (0.5 * Math.cos(x * Math.PI) + 0.5);
		}
	}
